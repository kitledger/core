...
import { WorkerToHostMessage, ExecutionResultPayload } from './shared.ts';
import { apiMethodMap, getApiMethod } from '../api/api.ts';
                ...
                switch (message.type) {
                    case 'actionRequest': {
                        try {
                            ...
                            hostPort.postMessage({ type: 'actionResponse', payload: { id: message.payload.id, result } });
                        } catch (e) {
                            hostPort.postMessage({ type: 'actionResponse', payload: { id: message.payload.id, error: (e as Error).message } });
                        }
                        break;
                    }
