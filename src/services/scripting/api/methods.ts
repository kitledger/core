...
import type { ApiMethod, ApiMethods } from "@kitledger/actions/__definition";
import type { HttpBodyRequestOptions, HttpRequestOptions, HttpResponse } from "@kitledger/actions/http";
...
export const apiMethodMap: ApiMethods = {
	...
	"log.audit": async (message, context) =>
		console.log(`%c[User Script | AUDIT]: ${message}`, "color: blue;", context ?? ""),
	"log.emergency": async (message, context) =>
		console.error(`%c[User Script | EMERGENCY]: ${message}`, "color: red;", context ?? ""),
